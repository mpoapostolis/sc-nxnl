    ...
    fun generate(analysis: SkinAnalysis): Routine {
        ...
        val evening = buildList {
            ...
            if (Concern.ACNE in weak || Concern.OILINESS in weak) {
                add(RoutineStep("Salicylic acid (BHA)", "2-3x a week — clears pores and excess oil", daily = false))
            }
            ...
            if (Concern.FINE_LINES in weak) {
                add(RoutineStep("Retinol", "Start 1-2x a week — smooths fine lines over time", daily = false))
            }
